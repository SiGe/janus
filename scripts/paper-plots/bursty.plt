...
plot "<cat bursty.data | grep '0\.64[[:space:]]\\|0\\.66[[:space:]]'" using 3 title "Janus" ls 2 lw 1, \
     "<cat bursty.data | grep '0\.64[[:space:]]\\|0\\.66[[:space:]]'" using 4 title "MRC" ls 1 lw 1, \

set yrange [0:50]
set ytics 0,10,60
set output "bursty-cost-74.pdf"
plot "<cat bursty.data | grep '0\.74[[:space:]]'" using 3 title "Janus" ls 2 lw 1, \
     "<cat bursty.data | grep '0\.74[[:space:]]'" using 4 title "MRC" ls 1 lw 1, \

set yrange [0:90]
set ytics 0,20,100
set output "bursty-cost-82.pdf"
plot "<cat bursty.data | grep '0\.82[[:space:]]\\|0\.80[[:space:]]'" using 3 title "Janus" ls 2 lw 1, \
     "<cat bursty.data | grep '0\.82[[:space:]]\\|0\.80[[:space:]]'" using 4 title "MRC" ls 1 lw 1, \

set yrange [0:140]
set ytics 0,20,140
set output "bursty-cost-90.pdf"
plot "<cat bursty.data | grep '0\.90[[:space:]]\\|0\.88[[:space:]]'" using 3 title "Janus" ls 2 lw 1, \
     "<cat bursty.data | grep '0\.90[[:space:]]\\|0\.88[[:space:]]'" using 4 title "MRC" ls 1 lw 1, \
