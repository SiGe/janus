               ...
               :{ /* network_watchtower_gen total:     92 6.1e-05 */
               :    /* Initialize network */
               ...
               :    {
     5 3.3e-06 :        for (int dst = 0; dst < tot_tors; dst++)
               :        {
               :            if (src == dst)
               :                continue;
    39 2.6e-05 :            int src_pod = src / t_per_p;
     2 1.3e-06 :            int dst_pod = dst / t_per_p;
    24 1.6e-05 :            if (src_pod == dst_pod)
               :            {
               :                *routing = 2;
     2 1.3e-06 :                *(routing+1) = 2 * src;
               :                *(routing+2) = 2 * dst + 1;
               :            }
               :            else
               :            {
               :                *routing = 4;
               :                *(routing+1) = 2 * src;
     9 6.0e-06 :                *(routing+2) = 2 * (src_pod + tot_tors);
    11 7.3e-06 :                *(routing+3) = 2 * (dst_pod + tot_tors) + 1;
               :                *(routing+4) = 2 * dst + 1;
               :            }
    ...
    22 1.5e-05 :{ /* network_reset total:   1267 8.4e-04 */
               :    network->fixed_flow_end = 0;
     1 6.6e-07 :    network->fixed_link_end = 0;
               :    network->smallest_flow = NULL;
               :    network->smallest_link = NULL;
               :    struct link_t *link = 0;
   260 1.7e-04 :    for (int i = 0; i < network->num_links; i++)
               :    {
   228 1.5e-04 :        link = network->links + i;
   265 1.8e-04 :        link->used = 0;
   150 9.9e-05 :        link->nactive_flows = 0;
    76 5.0e-05 :        link->nflows = 0;
    29 1.9e-05 :        free(link->flows);
   198 1.3e-04 :        link->flows = NULL;
    22 1.5e-05 :        link->next = NULL;
     9 6.0e-06 :        link->prev = NULL;
               :    }
     7 4.6e-06 :}
               :
               ...
               :}
/* 
 * Total samples for file : "/home/jiaqi/planning_config_dic/src/progressive-filling/topo.c"
 * 
 *   1359 9.0e-04
 */
