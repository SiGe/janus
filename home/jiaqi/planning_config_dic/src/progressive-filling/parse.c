   ...
   771 5.1e-04 :  for (;isspace(*input); input++){}
               :  return input;
               :}
               :
               :#ifdef WITH_SCANF
               ...
               :}
               :
  1090 7.2e-04 :int eat_bw(char const *input, bw_t *value) { /* eat_bw total:   4600  0.0030 */
               :  char const *begin = input;
               :  input = strip_space(input);
               :  bw_t result = 0;
  1379 9.1e-04 :  for(;isdigit(*input);input++){
   546 3.6e-04 :    result = (result * 10) + ((*input) - '0');
               :  }
     2 1.3e-06 :  *value = (bw_t)result;
               :
               :  bw_t fraction = 0;
               :  bw_t power = 1.0;
   281 1.9e-04 :  if (*input == '.'){
               :    input++;
               :  }
   271 1.8e-04 :  for (;isdigit(*input); input++){
               :    fraction += (((*input) - '0') / power);
               :    power *= 10;
               :  }
   257 1.7e-04 :  *value += fraction;
     1 6.6e-07 :  return (input - begin);
     2 1.3e-06 :}
               :
               :int read_uint32(char const *input, uint32_t *value, int *pos) {
               ...
               :}
               :
   566 3.7e-04 :int read_bw(char const *input, bw_t *value, int *pos) { /* read_bw total:   1331 8.8e-04 */
   248 1.6e-04 :  *pos = eat_bw(input, value);
               :  if (*pos == 0) return 0;
               :  return 1;
   517 3.4e-04 :}
               :#endif
               ...
               :}
/* 
 * Total samples for file : "/home/jiaqi/planning_config_dic/src/progressive-filling/parse.c"
 * 
 *   5931  0.0039
 */
