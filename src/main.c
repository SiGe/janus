...
rvar_type_t _sim_network_for_trace(void *data) {
  ...
  /*
  bw_t total = 0;
  for (uint32_t i = 0; i < tm->num_pairs; ++i) { total += tm->bws[i].bw; }

  bw_t flows_tot = 0;
  for (uint32_t i = 0; i < builder->dp.num_flows; ++i) { flows_tot += builder->dp.flows[i].bw ; }

  bw_t links_tot = 0;
  for (uint32_t i = 0; i < builder->dp.num_links; ++i) { links_tot += builder->dp.links[i].capacity ; }

  info("State is :%d, tm: %f, flows: %f, links: %f, violations: %d, percentage: %f", builder->iter->state, total, flows_tot, links_tot, violations, percentage);
  */
