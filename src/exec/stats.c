...
void _exec_stats_runner(struct exec_t *exec, struct expr_t *expr) {
  ...
  for (uint32_t i = 0; i < npods; ++i) {
    info("Pod  [%2d] stats: (%14.2f, %14.2f, %14.2f)", pods[i].id, pods[i].in.min, pods[i].in.mean, pods[i].in.max);
  }
  info("Core [  ] stats: (%14.2f, %14.2f, %14.2f)", core->in.min, core->in.mean, core->in.max);

  info("OUT -----");
  for (uint32_t i = 0; i < npods; ++i) {
    info("Pod  [%2d] stats: (%14.2f, %14.2f, %14.2f)", pods[i].id, pods[i].out.min, pods[i].out.mean, pods[i].out.max);
  }
