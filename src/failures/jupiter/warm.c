...
static double _prob_for_failure(unsigned *tuple, struct mop_block_stats_t *blocks, 
  ...
  // The list of switches that can failre are the switches that are getting
  // upgraded
  for (int i = 0; i < nblocks; ++i) {
    uint64_t free_sw = blocks[i].down_switches;
    if (tuple[i] > free_sw)
      return 0;
...
static double _failure_prob_of_k_concurrent_sws(
    struct jupiter_failure_model_warm_t *jm,
    struct jupiter_failure_warm_iterator_t *ji) {
  uint64_t nfreesw = ji->nfreesw;
  uint64_t naf = ji->num_active_failures;
  if (nfreesw < naf) return 0;
  ...
  return nstates * pow(1-sfp, nfreesw - naf) * pow(sfp, naf);
}
...
static uint32_t _lds_for_failure(unsigned *tuple, struct mop_block_stats_t *blocks, 
  ...
  unsigned *nds =  malloc(sizeof(unsigned) * nblocks);
  for (int i = 0; i < nblocks; ++i) {
    /* The topology resulting from a WARM reboot failure equals the original
     * topology + the failures */
    nds[i] = blocks[i].down_switches;
    blocks[i].down_switches = tuple[i];

    // TODO: This assertion should be true cause in reality that block only has
    // tuple[i] /warm/ failures
...
#define TO_JI(p) struct jupiter_failure_warm_iterator_t *ji =\
  (struct jupiter_failure_warm_iterator_t *)(p)

#define TO_JM(p) struct jupiter_failure_model_warm_t *jm =\
  (struct jupiter_failure_model_warm_t*)(p)
...
static void _jii_begin(struct failure_scenario_iterator_t *fi) {
  ...
  for (int i = 0; i < nblocks; ++i) {
    ...
    // Only warm switches can fail
    nfreesw += (blocks[i].down_switches);
  }
...
static void _jii_free(struct failure_scenario_iterator_t *fi, struct rvar_t **rvars, int nvars) {
  if (rvars && nvars > 0) {
    for (int i = 0; i < nvars; ++i) {
      rvars[i]->free(rvars[i]);
    }
  }
...
static struct rvar_t *_jii_cost(struct failure_scenario_iterator_t *fi) {
  ...
  struct rvar_t *cost_rv = rvar_fixed(ji->num_active_failures * jm->failure_cost);
  struct rvar_t *ret = cost_rv->convolve(cost_rv, ji->rcache[lds_id], 1);
  cost_rv->free(cost_rv);
  return ret; //ji->rcache[lds_id];
}

static struct failure_scenario_iterator_t *_jupiter_warm_iter(
    struct failure_model_t const *fm,
    struct mop_block_stats_t *blocks,
    uint32_t nblocks,
    struct plan_iterator_t *pi,
    struct rvar_t **rcache) {
  size_t size = sizeof(struct jupiter_failure_warm_iterator_t);
  struct jupiter_failure_warm_iterator_t *iter = malloc(size);
  memset(iter, 0, size);
...
jupiter_failure_model_warm_create (uint32_t max_concurrent_failure,
    double switch_failure_probability, double failure_cost) {
  struct jupiter_failure_model_warm_t *ret = 
    malloc(sizeof(struct jupiter_failure_model_warm_t));

  ret->apply = failure_default_apply;
  ret->iter = _jupiter_warm_iter;
  ret->max_concurrent_switch_failure = max_concurrent_failure;
  ret->switch_failure_probability = switch_failure_probability;
  ret->failure_cost = failure_cost;
  return ret;
}
